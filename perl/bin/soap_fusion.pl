...
use PCAP::Cli;
use Sanger::CGP::Soap::Implement;

my $ini_file = "$FindBin::Bin/../config/soap.ini"; # default config.ini file path
const my @REQUIRED_PARAMS => qw(outdir sample readlength library);
const my @VALID_PROCESS => qw(prepare soapsetup soap filter);
...
print Dumper(\$options);
	if(!exists $options->{'process'} || $options->{'process'} eq 'prepare'){
		...
		$threads->add_function('prepare', \&Sanger::CGP::Soap::Implement::prepare);
		$threads->run($options->{'max_split'}, 'prepare', $options);
	}

	Sanger::CGP::Soap::Implement::soap_setup($options) if(!exists $options->{'process'} || $options->{'process'} eq 'soapsetup');
	Sanger::CGP::Soap::Implement::soap($options) if(!exists $options->{'process'} || $options->{'process'} eq 'soap');
	
	if(!exists $options->{'process'} || $options->{'process'} eq 'filter'){
		Sanger::CGP::Soap::Implement::filter_fusions($options);
		cleanup($options);
	}
}
...
sub setup {
	...
	GetOptions( 	'h|help' => \$opts{'h'},
			...
			'rl|readlength=i' => \$opts{'readlength'},
			'l|library=s' => \$opts{'library'},
			'sc|soapconfig=s' => \$opts{'soapconfig'},			
	...
	$opts{'refdataloc'} = $cfg->val('soap-config','referenceloc') unless(defined $opts{'refdataloc'});
	$opts{'referencebuild'} = $cfg->val('soap-config','referencebuild') unless(defined $opts{'referencebuild'});
	$opts{'genebuild'} = $cfg->val('soap-config','genebuild') unless(defined $opts{'genebuild'});
	$opts{'normalfusionslist'} = $cfg->val('soap-config','normalfusionslist') unless(defined $opts{'normalfusionslist'});
	$opts{'species'} = $cfg->val('soap-config','species') unless(defined $opts{'species'});
	$opts{'soapconfig'} = $cfg->val('soap-config','soapconfig');
	$opts{'soappath'} = $cfg->val('soap-config','soappath');
	$opts{'soapversion'} = $cfg->val('soap-config','soapversion');

	# Print version information for this program
	if($opts{'version'}) {
		print 'CGP soap_fusion.pl version: ',Sanger::CGP::Soap::Implement->VERSION,"\n";
		print 'SOAPfuse version: ',$opts{'soapversion'},"\n" if(defined $opts{'soapversion'});
		exit 0;
	}
	...
	my $tmpdir = File::Spec->catdir($opts{'outdir'}, 'tmpSoap');
	...
	my $inputdir = File::Spec->catdir($tmpdir, 'input', $opts{'sample'}, $opts{'library'});
	...
	$opts{'raw_files'} = \@ARGV;
	Sanger::CGP::Soap::Implement::check_input(\%opts);
...
soap_fusion.pl [options] [file(s)...]

  Required parameters:
    -outdir    		-o   	Folder to output result to.
    -sample   		-s   	Sample name
    -library   		-l   	Sample library
    -readlength  	-rl   	Read length
  ...
  Optional
    ...
    -config   		-c  	Path to config.ini file. It contains defaults for; the reference and gene build versions, SOAPfuse software and default SOAPfuse parameters [<cgpRna-install-location>/perl/config/soap.ini]
  ...
  File list can be full file names or wildcard, e.g.
    soap_fusion.pl -t 8 -o myout -refbuild GRCh38 -genebuild 77 -s sample -rl 101 -l library_name input/*.bam
