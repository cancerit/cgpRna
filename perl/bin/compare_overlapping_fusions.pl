...
Produces a report of fusions that have been called by two algorithms - it will report at the gene and exon level.
...
compare_overlapping_fusions.pl [options] [file(s)...]
  ...
  Required parameters:
    ...
    -threads    	-t   	Number of threads (cpus) to use.
    
    Files can be filtered output generated by cgpRna pipeline; defuse.pl, tophat_fusion.pl or star_fusion.pl
