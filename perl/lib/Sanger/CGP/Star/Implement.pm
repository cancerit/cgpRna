...
sub check_input {
  ...
  my $fusion_mode;
	...
	my $gtf;
	if(defined $options->{'gtffilename'} && -e $options->{'gtffilename'}) {
		$gtf = $options->{'gtffilename'};
	}
	else {
		$gtf = File::Spec->catfile($options->{'refdataloc'},$options->{'species'},$options->{'referencebuild'}, 'star', $options->{'genebuild'}, $options->{'gtffilename'});
	}
	PCAP::Cli::file_for_reading('gtf-file', $gtf);
	$options->{'gtffilename'} = $gtf;
...
sub process_star_params {
	...
	my $gtf = $options->{'gtffilename'}; # fixed in check_inputs

	my $star_index;
	if(defined $options->{'staridx'} && exists $options->{'staridx'}) {
		$star_index = $options->{'staridx'};
	}
	else {
		$star_index = File::Spec->catdir($options->{'refdataloc'},$options->{'species'},$options->{'referencebuild'}, 'star');
	}
