...
def validate_input_seq_files(file_names):
    ...
    for a_file in file_names:
        ...
        match = re.match(r'(.*)_([12])\.f(?:ast)?q(?:\.gz)?$', os.path.basename(a_file))
        if match.group(2):
            # if same mate number been spotted before or already got both mates
            if pairs.get(match.group(1), 0) == int(match.group(2)) or pairs.get(match.group(1), 0) == 3:
                sys.exit('Error: Too many \'_%s\' mate files for prefix: %s. Possibly redundant file: %s' % (match.group(2), match.group(1), a_file))
            pairs[match.group(1)] = pairs.get(match.group(1), 0) + int(match.group(2))
        else:
            sys.exit('Error: File name does not follow expected coventions: %s' % a_file)
    for a_mate, value in pairs.items():
        # if both _1 and _2 of a pair of fastq files are give, value should be 3
        if value != 3:
            if value == 1:
                sys.exit('Error: Can not find second mate file of: %s' % a_mate)
            if value == 2:
                sys.exit('Error: Can not find first mate file of: %s' % a_mate)
            sys.exit('Error: Internal code logic error')
...
def run_fusion_wrapper(args, temp_dir_name, fusion_templates, no_gzip=False):
    ...
    for a_raw_file in args.input:
        # if input is not a bam, assume it's a fastq. i.e not supporing CRAM.
        if not a_raw_file.endswith('.bam'):
            if no_gzip and a_raw_file.endswith('.gz'):
                # dumb Defuse perl wrapper doesn't like gzipped, so:
                unzip_to = os.path.join(temp_dir, re.sub(r'\.gz$', '', os.path.basename(a_raw_file)))
